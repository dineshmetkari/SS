...
//	@Autowired
//	public void setFetchConceptIntentRepository(FetchConceptIntentRepository fetchConceptIntentRepository) {
...
//		concepts = fetchConceptIntentRepository.getConcepts();
	...
	public List<String> fetchParentNodes(){
		Config config = new Config();
	    config.useSingleServer().setAddress("redis://172.23.238.178:6379");
	    RedissonClient redisson = Redisson.create(config);
	    RBucket<Neo4jParentNlpModel> bucket = redisson.getBucket("parentNlpModel");
	    List<String> parentNodes = bucket.get().getParentNodes();
		return parentNodes;
	}
