...
public class MainService {
	...
	public List<JSONObject> getConfidenceScore(IncomingFormat format){
		...
		/**
		 * objList contains input from orchestration service with all terms
		 * and its scores
		 */
		List<JSONObject> objList = new ArrayList<JSONObject>();
		/**
		 * neofetch list contains json object of intent, word and 
		 * its weight in intent graph
		 */
		...
		while(a.hasNext()) {
			...
			if(value!=0)
			{
				...
				for(NeoFetch neo:neoFetchList)
				{
					...
					{
						System.out.println("In neo4j"+neo.getName()+" "+neo.getWeight());
		...
		//        }
		//        
		//        else if((probIntermediate>probBeginner)&&(probIntermediate>probAdvance)&&(probIntermediate>probIllustration))
		//        {
		//        	confidenceScore=probIntermediate;
		//        	intent="Intermediate";       	
		//        } 
		//        
		//        else if((probAdvance>probBeginner)&&(probAdvance>probIntermediate)&&(probAdvance>probIllustration))
		//        {
		//        	confidenceScore=probAdvance;
		//        	intent="Advance";
		//        }
		//        
		//        else if((probIllustration>probBeginner)&&(probIllustration>probIntermediate)&&(probIllustration>probAdvance))
		//        {
